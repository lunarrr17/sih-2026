...
document.getElementById('triageForm').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    const licensingForms = data.licensing_forms || data.forms_required || [];
    forms.innerHTML = licensingForms.map(f => `<span class="verdict-pill neutral">${f}</span>`).join('');
...
window.openCitation = (id) => {
  ...
  const pageInfo = (d.page_numbers && d.page_numbers.length > 0)
    ? `<div class="statute">Page(s): ${d.page_numbers.join(', ')}</div>`
    : '';
...
document.getElementById('exportDossierBtn').addEventListener('click', async () => {
  ...
  try {
    const prodName = document.getElementById('triageProductName') ? document.getElementById('triageProductName').value : 'Ayurvedic_Formulation';
    const catName = document.getElementById('catBadge') ? document.getElementById('catBadge').innerText : 'Classical Medicine';
    const citationsList = Object.values(citeDataStore || {});
    ...
    const res = await fetch(`${API_BASE}/api/export/dossier`, {
      ...
      body: JSON.stringify({
        ...
        jurisdiction: currentJurisdiction,
        chat_history: [
          { role: 'user', content: 'Compliance inquiry' },
          { role: 'bot', content: 'Evaluation dossier' }
        ],
